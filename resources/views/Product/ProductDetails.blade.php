...
{{--    View Details Modal--}}
    <div class="modal animate__animated animate__zoomIn" id="viewDetails" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <p class="modal-title" id="exampleModalLabel">Details Data</p>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
@section('script')
    <script>
                ...
                if(response.status==200){
                    ...
                    $.each(response.data,function (i,item){
                        let tableRow="<tr>" +
                            ...
                            "<td> " + "<span  data-eid="+item['id']+" style='font-size:25px;'  class='VewShortDescBtn text-dark'><i class='fas fa-eye'></i></span> " + "</td>"+
                            "<td>"+item['color']+"</td>" +
                            "<td>"+item['size']+"</td>" +
                            "<td> " + "<span  data-eid="+item['id']+" style='font-size:25px;'  class='VewDetailsBtn text-dark'><i class='fas fa-eye'></i></span> " + "</td>"+
                            "<td>" +
                            "<div class='dropdown'>" +
                    ...
                    $('.VewShortDescBtn').on('click',function () {
                        let idEdit=$(this).data('eid');
                        $('#EditID').html(idEdit);
                        GetShortDescData(idEdit);
                        $('#viewDetails').modal('show');
                    })
                    $('.VewDetailsBtn').on('click',function () {
                        let idEdit=$(this).data('eid');
                        $('#EditID').html(idEdit);
                        GetDetailsData(idEdit);
                        $('#viewDetails').modal('show');
                    })
        ...
        //details data code
        function GetShortDescData(idDetails){
            ...
            axios.post(URL,{id:idDetails},AxiosConfig).then(function (response) {
                ...
                if (response.status===200) {
                    ...
                    $('#detailsDataView').html(EditData[0]['des']);
        ...
        function GetDetailsData(idDetails){
            ...
            axios.post(URL,{id:idDetails},AxiosConfig).then(function (response) {
                ...
                if (response.status===200) {
                    ...
                    $('#detailsDataView').html(EditData[0]['details']);
                }
                else {
                    $('#viewDetails').modal('hide');
                    ErrorToast("Something went wrong !");
